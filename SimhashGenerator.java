...
public class SimhashGenerator {
    public static class LineSimhash {
        ...
        public final long CombinedSimhash;// Combines both ContentSimhash and CombinedSimhash forming a single value
                                          // 64bits
        /*---------Constructor----------*/
        ...
        public LineSimhash(int index, long contentSimhash, long contextSimhash) {
            ...
            this.CombinedSimhash = contentSimhash ^ contextSimhash;// combines two hashes XOR (^)combines two Simhashes
                                                                   // without increasing size which later to be used to
        ...
        /*
         * We normalize again in Step 3 because Simhash is very sensitive to tiny
         * differences.
        ...
        // since Context is enough so we ExtracContext that we later feed Simhash to
        // help match lines more accurately.
        ...
        public String extractContext(List<String> lines, int index, int windowSize) {
            ...
            StringBuilder sb = new StringBuilder();// StringBuilder one BIg String : will contain all neighbor lines
                                                   // joined together.
        ...
        /// This method computes a 64-bit SimHash for a piece of text:similar texts
        /// produce similar 64-bit values
        public long computeSimhash(String text) {
            // If string is missing or empty, the method returns 0 because no hash can be
            // computed
            if (text == null || text.isEmpty()) {//
                return 0L;
            }
            /*
             * even though we have method extractContext save the line in sb(as one big line
             * with spaces )
             * we still split into tokens : each words needs to treated independently as
             * SimHash is designed to combine many small token-hashes, not one giant hash.
            ...
            for (String token : tokens) {
                ...
                long hash = hash64(token); // Using .util.* library: string gets turned into a 64 bit number we get the
                                           // numeric fingerprint” of that word.
                // *-----SimHashlook does not read all 64 bits at once so looked at each bit at
                // time so it needs to update the vector one bit at a time.--------/
                for (int bit = 0; bit < 64; bit++) {

                    // : 1L is just 1 stored in a long type
                    // << (left shift operator)take the bits of a and shift them to the left by n
                    // positions

                    // A bitmask → creating a number that has only one bit turned on all other
                    // values are zero other than one 1
                    long bitmask = 1L << bit;
                    if ((hash & bitmask) != 0) {//
                        /*
                         * It compares each bit: AND keeps only the bit that both numbers have ON for
                         * not zero
        ...
        public long hash64(String s) {
            ...
            for (int i = 0; i < len; i++) {
                 ...
                 * Multiplying h by 31 moves the old hash to a higher place so we can add the
                 * new character in.
                 */
        ...
        public Map<Integer, List<Candidate>> generateCandidates(
            ...
            for (LineSimhash oldLineHash : oldSimhashes) {
                ...
                // worst candidate is always on top keep only best (most similar) new lines for
                // each old line.
                // when found a better one, we kick out the worst one.
                PriorityQueue<Candidate> maxHeap = new PriorityQueue<>(k,
                        (a, b) -> Integer.compare(b.hammingDistance, a.hammingDistance));
